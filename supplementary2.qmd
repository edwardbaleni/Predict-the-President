...
# Since first sentence is similar for each president
              ...
              # Removing the first sentence actually makes classification worse
...
tidy_tweets <- data_count %>%
  unnest_character_shingles(token, speech, n = 4) #%>%         # Somehow, not removing tokens works in identifying st
  #filter(!token %in% stop_words$word)
...
<!-- 0.46 misscalssification achieved when nchars is 3, and all tokens are kept (No word bag) -->
<!-- 0.44 with nchars is 4 and 2000 words kept -->
<!-- 0.51 with nchars is 4 and all tokens kept -->
<!-- 0.46 with nchars is 3, 2000 tokens and punctuation is included  -->
...
# library(rminer)
# M <- fit(Response_President~., data=training_tweets, model="svm", kpar=list(sigma=0.10), C=4, scale = F)
# svm.imp <- Importance(M, data=training_tweets)
```

```{r FFNN}
y_train <- as.integer(unlist(training_tweets[,1]))
x_train <- as.matrix(training_tweets[,-1])

y_test <- as.integer(unlist(test_tweets[,1]))
x_test <- as.matrix(test_tweets[,-1])
...
model <- keras_model_sequential() %>%
   layer_dense(units = 8, activation = 'relu', input_shape = c(8166)) %>% 
   layer_dropout(rate = 0.5) %>%
...
results <- model %>% evaluate(x_test, y_test, batch_size=1000, verbose = 2)
results
```
