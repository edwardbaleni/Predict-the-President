...
```{r Cleaning Data}
sona$speech <- sapply(sona$speech, replace_number)#replace_number(sona$speech)  #gsub('[0-9]+', '', sona$speech)
sona$speech <- sapply(sona$speech, replace_contraction)#replace_contraction(sona$speech)
sona$speech <- sapply(sona$speech, replace_symbol)#replace_symbol(sona$speech)
...
test <- sentences %>%
  ...
  anti_join(train, by = join_by(token))
```
...
```{r}
a <- uni_tf_idf %>%
  cast_sparse(president, token, tf_idf)
...
wordcloud(train$token, max.words = 100, colors = c("grey80", "darkgoldenrod1", "tomato"), min.freq = 10, random.order = FALSE)
```

```{r}

# Create a document term matrix
test_dtm <- DocumentTermMatrix(Data_test_corpus_clean, control = list(weighting =
                                         function(x)
...
library(e1071)
data_classifier <- naiveBayes(data_train, Data_train$president, laplace = 1)
