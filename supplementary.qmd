...
```{r 3 Characters Bigger Bag No Punc}
uni <- data_count %>%
  unnest_character_shingles(token, speech, n = 3, strip_non_alphanum = F) 
...
```{r 4 Characters Bigger Bag No Punc}
bi <- data_count %>%
...
```{r 3 Char Test Bigger Bag No Punc}
train.set    <- bag.of.words.uni[bag.of.words.uni$ids_ %in% train, -1]
val.set      <- bag.of.words.uni[bag.of.words.uni$ids_ %in% val, -1]
test.set     <- bag.of.words.uni[bag.of.words.uni$ids_ %in% test, -1]
...
1 - sum(diag(tt))/ sum(tt)
```
<!-- For nchar = 4, when bag size = 3000, naive bayes = 0.47 and svm = 0.58 -->
...
At this stage it is hard to say whether the inclusion of punctuation is adding or removing value from the analysis.
