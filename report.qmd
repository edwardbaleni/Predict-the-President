...
The data used in this study was sourced from [gov.za](https://www.gov.za/state-nation-address). As mentioned adresses from 1994 to 2023 are used within this study. The speakers present in this time span were FW De Klerk, Nelson Mandela, Thabo Mbeki, Kgalema Motlanthe, Jacob Zuma and Cyril Ramaphosa. Some webscraping techniques, courtesy of Ian Durbach, were used to collect this data.
...
TF-IDF is the multiplication of the two metrics ([\@TextMining]()). TF-IDF measures the importance of a word to a document in a collection. In this analysis this has been done by considering one document to be a sentence. Following this intuition, the quantities above can be calculated.

In the [Supplementary](supplementary.qmd), both the lexical feature approach and the character features were looked into to find which would be better performing for TF-IDF. The character features seemed to show a better result and so will be used in this report.
...
A multinomial naive Bayes classifier was use to classify text. This is a probabilistic classifier, given the document $d$ and classes $c \in C$, the classifier will predict the class, $\hat{c}$, with the maximum posterior probabiliity,
...
Support vector machines, SVM, are widely used in the space of text classification ([\@Stylometry]()). SVMs perform linear operations in a higher-dimensional manifold.

<!-- Explain this tomorrow after lecture -->
...
The activation for the hidden layers has been chosen as the reLu activation function and the output layer uses a soft-max activation function. The classification task uses accuracy rate to evaluate model performance. Like the other models this is just a measure of correctly classified predicitons.

<!-- Talk about forward and backward passes -->

To find the best model in terms of feed forwards neural networks, the bag of words at different word bags was assessed against a default model with 32 nodes in the input layer with a relu activation function, a dropout rate of 0.5 at this layer, and an output layer with a softmax activation function to evaluate a categorical classification. Later on, hyperparameter tuning to find a better model will happen and used to test the final data.
```{r Data for FFNN}
...
# TFIDF naive bayes
1 - sum(diag(tfidf.t))/ sum(tfidf.t)

# Lexical SVM
1 - sum(diag(svm.t.1.1))/ sum(svm.t.1.1)
...
# Character SVM
1 - sum(diag(svm.t.1.2))/ sum(svm.t.1.2)
...
# TFIDF SVM
1 - sum(diag(svm.tfidf.t))/ sum(svm.tfidf.t)

# Lexical FFNN
1 - model.1.1 %>% evaluate(x.val.1.1, y.val.1.1, batch_size=100, verbose = 2)
1 - model.1.2 %>% evaluate(x.val.1.2, y.val.1.2, batch_size=100, verbose = 2)
1 - model.1.3 %>% evaluate(x.val.1.3, y.val.1.3, batch_size=100, verbose = 2)
1 - model.1.4 %>% evaluate(x.val.1.4, y.val.1.4, batch_size=100, verbose = 2)
1 - model.1.all %>% evaluate(x.val.1.all, y.val.1.all, batch_size=100, verbose = 2)

# Character FFNN
1 - model.2.1 %>% evaluate(x.val.2.1, y.val.2.1, batch_size=100, verbose = 2)
1 - model.2.2 %>% evaluate(x.val.2.2, y.val.2.2, batch_size=100, verbose = 2)
1 - model.2.3 %>% evaluate(x.val.2.3, y.val.2.3, batch_size=100, verbose = 2)
1 - model.2.4 %>% evaluate(x.val.2.4, y.val.2.4, batch_size=100, verbose = 2)
1 - model.2.all %>% evaluate(x.val.2.all, y.val.2.all, batch_size=100, verbose = 2)

# TFIDF FFNN
  # Best model is TFIDF with characters
1 - model.tfidf %>% evaluate(x.val.tfidf, y.val.tfidf, batch_size=100, verbose = 2)
```

## Final Model

<!-- Use test set on final model -->
