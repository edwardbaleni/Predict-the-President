...
In Table 3 we see that the best hyperparameters for the model built off the character feature set are in fact the same as the default model described at the beginning of the paper. This table depicts the accuracy of the model, which is,
$$\text{accuracy} = 1 - \text{missclassification}.$$
So a high accuracy is good.
...
as_hux(runs1.1) %>%
  ...
  set_caption("Table 3: Cross validated results depicting the best hyperparameters for the character feature set in a single layer FFNN")
```
...
For each model, we can begin by combining the training and validation data, then we can go ahead and train this new and final model.
