...
bigram_tf_idf %>%
    ...
    top_n(15) %>%
    ungroup %>%
    mutate( token = reorder_within(token, tf_idf, president)) %>%
    ggplot(aes(token, tf_idf, fill = president)) +
    geom_col(show.legend = FALSE) +
...
## Convolutional Neural Networks or Tree-Based Method or (whatever the book suggests) or Logisitic Regression
