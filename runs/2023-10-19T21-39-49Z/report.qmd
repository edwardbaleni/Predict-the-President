...
A single layer FFNN has been chosen. Hyperparameter tuning will be done on the hidden layer to see if 10, 32 nodes or 64 nodes is better, and to see if changing the learning rate of the adam optimizer will result in a better model. The learning rates used are 0.1 and 0.5.
...
```
Earlier when looking for a goof feature set, we clearly saw that 50 epochs was too many, or that the batch size was either too great or too small
